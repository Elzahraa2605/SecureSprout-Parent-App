...
import {
  ...
  TouchableOpacity,
  View,
  Image // 👈 تم استيراد الـ Image لعرض الأيقونات الحقيقية للتطبيقات
} from 'react-native';
...
export default function ManageApps() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const response = await axios.get('http://16.171.208.58/api/parent/childs', { headers });
  ...
  const fetchApps = async (childId: number, token: string) => {
    try {
      setRefreshing(true);
      const headers = { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' };
      const res = await axios.get(`http://16.171.208.58/api/parent/child-device-apps/${childId}`, { headers });
  ...
  const updateSettings = async (appId: number, updateData: any) => {
    try {
      ...
      const res = await axios.patch(`http://16.171.208.58/api/parent/child-apps/update/${appId}`, formattedData, { headers });
            ...
            <View key={app.id}>
              <View style={styles.appItem}>
                <View style={styles.appMainInfo}>
                   ...
                   {/* 👈 تعديل هذا البوكس لعرض الصورة الحقيقية المستقلة القادمة من السيرفر للتطبيق */}
                   <View style={styles.appIconBoxCustom}>
                      <Image 
                        source={{ 
                          uri: app.app_icon && app.app_icon.startsWith('http') 
                            ? app.app_icon 
                            : 'https://via.placeholder.com/50' 
                        }}
                        style={styles.imageIconStyleCustom}
                        resizeMode="contain"
                      />
                   </View>
...
const styles = StyleSheet.create({
  ...
  // ستايلات مخصصة ومستقلة لحاوية أيقونة الأبلكيشن لمنع الخلط
  appIconBoxCustom: { width: 45, height: 45, backgroundColor: '#E3F2FD', borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginRight: 12, overflow: 'hidden' },
  imageIconStyleCustom: { width: '100%', height: '100%' },
