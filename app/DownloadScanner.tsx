...
const BACKEND_URL = 'http://16.171.208.58:9000';
...
export default function DownloadScanner() {
    ...
    try {
      ...
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE,
        {
          ...
          message:        'App needs storage access to scan the downloads folder',
  ...
  const loadFiles = useCallback(async () => {
    if (!isModuleAvailable) {
      setLoading(false);
      Alert.alert('Warning', 'Please add FileMonitorPackage in MainApplication.kt first');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Error', 'Could not load file list: ' + String(e));
    }
    setLoading(false);
  }, [requestPermission]);
  ...
  const scanAll = useCallback(async () => {
    ...
    try {
      ...
      if (blocked > 0) {
        Alert.alert(
          '⚠️ Warning!',
          `${blocked} dangerous file(s) detected and removed automatically from the downloads folder.`,
          [{ text: 'OK' }]
        );
      } else {
        Alert.alert('✅ Safe', `Scanned ${safe} file(s) — everything is clean`);
      }

    } catch (e) {
      Alert.alert('Error', 'Scan failed: ' + String(e));
    }
    setScanning(false);
    setProgress(null);
    loadFiles();
    ...
    return (
      <View style={s.progressContainer}>
        <View style={s.progressHeader}>
          <Text style={s.progressTitle}>🔬 Scanning...</Text>
    ...
    return (
      <View style={[s.fileCard, isBlocked && s.fileCardBlocked, isSafe && s.fileCardSafe]}>
        ...
        <View style={s.fileInfo}>
          ...
          {isPending  && <Text style={s.filePending}>⏳ Not scanned yet</Text>}
          {isSafe     && <Text style={s.fileSafe}>Clean ✓</Text>}
        </View>
    ...
    <View style={s.container}>
      ...
      <View style={s.stats}>
        <View style={[s.stat, { backgroundColor: '#e8f5e9' }]}>
          ...
          <Text style={s.statLbl}>✅ Safe</Text>
        </View>
        ...
        <View style={[s.stat, { backgroundColor: '#ffebee' }]}>
          ...
          <Text style={s.statLbl}>☣️ Dangerous</Text>
        </View>
      ...
      >
        {scanning
          ...
          : <Text style={s.scanBtnTxt}>🔬 Scan All Now</Text>
        }
      </TouchableOpacity>
      ...
      {loading ? (
        ...
        <View style={s.empty}>
          ...
          <Text style={s.emptyTxt}>Downloads folder is empty</Text>
        </View>
      ) : (
        <FlatList
          data={files}
