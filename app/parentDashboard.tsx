...
const API_BASE_URL = 'http://16.171.208.58/api'; 
...
export default function ParentDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // --- تم التعديل هنا: استخراج الاسم من الإيميل المخزن في AsyncStorage ---
      const userEmail = await AsyncStorage.getItem('userEmail');
      if (userEmail) {
        const namePart = userEmail.split('@')[0];
        const formattedName = namePart.charAt(0).toUpperCase() + namePart.slice(1);
        setParentName(formattedName);
      }
      // -------------------------------------------------------------------
  ...
  const fetchLaravelAlerts = useCallback(async (childId: number | string, token: string) => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        // 🎯 الحركة السحرية: استبعاد تنبيهات البكاء واختيار التنبيهات الأمنية الخاصة بالطفل فقط (موقع أو ملف)
        const securityAlertsOnly = response.data.filter((alert: any) => 
          alert.type !== 'crying_detected' && alert.type !== 'Cry'
        );

        if (securityAlertsOnly.length > 0) {
          setLatestAlert(securityAlertsOnly[0]); // عرض أحدث تنبيه أمني حقيقي في البوكس الأصفر
        } else {
          setLatestAlert(null);
        }
      } else {
