...
export default function ParentSignUp() {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://16.171.208.58/api/parent/register', {
        name: fullName,
        email: email,
        password: password,
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      {/* ... نفس كود الـ UI ... */}
      {/* زر الرجوع المحدث على شكل سهم جهة اليسار */}
...
// 🛠️ تحديث ستايل الزر لينزل لأسفل ويكون سهل الضغط
  backButton: { 
    alignSelf: 'flex-start',
    marginTop: 15,        // 👈 مسافة أمان إضافية من الأعلى لتبتعد عن شريط الهاتف
    marginBottom: 10, 
    padding: 10,          // 👈 لتوسيع منطقة الضغط بالإصبع
    marginLeft: -10       // لضبط المحاذاة جهة اليسار بدقة بسبب الـ padding
  },
