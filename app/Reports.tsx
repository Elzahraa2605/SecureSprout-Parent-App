...
// ⚠️ ملحوظة مهمة: تم حذف استدعاء AppListModule.getTodayUsage() من هذه الشاشة نهائياً.
// هذه الشاشة شاشة "الأب" فقط، وأي قراءة Native لإحصائيات الاستخدام (UsageStatsManager)
// تُنفَّذ على الجهاز الذي يشغّل الكود حالياً (جهاز الأب نفسه عند فتحه للتطبيق)
// وليس بالضرورة جهاز الطفل المختار. مزامنة بيانات الاستخدام الفعلية لازم تتم فقط
// من داخل AppMonitorService.kt الذي يعمل حصرياً على جهاز الطفل.
...
export default function Reports() {
  ...
  const fetchReports = async (childId: any, range: string) => {
    ...
    try {
      ...
      const response = await axios.get(`http://16.171.208.58/api/parent/reports/app-usage/${childId}?range=${range}`, { headers });
      ...
      const alertsResponse = await axios.get(`http://16.171.208.58/api/parent/alerts?child_id=${childId}&range=${range}`, { headers });
      const backendAlerts = Array.isArray(alertsResponse.data) ? alertsResponse.data : [];

      // 🔥 التصفية السحرية: استبعاد أي تنبيه يخص البكاء من شاشة الطفل
      const securityThreatsOnly = backendAlerts.filter((alert: any) => 
        alert.type !== 'crying_detected' && alert.type !== 'Cry'
      );

      // وضع التهديدات الأمنية فقط في قائمة العرض
      setSecurityAlerts(securityThreatsOnly);
  ...
  // ✅ زرار الريفرش بقى وظيفته الوحيدة إنه يعيد جلب التقارير من السيرفر.
  // مفيش هنا أي قراءة Native لإحصائيات استخدام أي جهاز، لأن هذه الشاشة تُفتح
  // على جهاز الأب، وأي قراءة Native هنا كانت بترجع بيانات استخدام جهاز الأب
  // نفسه وتُسجَّل خطأً على حساب الطفل المختار. مزامنة بيانات الاستخدام الحقيقية
  // بتتم تلقائياً في الخلفية من جهاز الطفل نفسه (راجع AppMonitorService.kt).
