...
export default function UserTypeScreen() {
    ...
    const checkUserStatus = async () => {
      try {
        ...
        if (userToken) {
          if (userType === 'child') {
            try {
              const response = await axios.get('http://16.171.208.58/api/child/me', {
          ...
          // ملحوظة: ما بنعملش auto-redirect للأب هنا، عشان زراير
          // Parent/Child تفضل تظهر دايماً. التحقق من حالة الأب
          // بيحصل لما يدوس بنفسه على زرار Parent (شوفي handleParentPress)
        }
        // لو مفيش توكن، أو المستخدم أب، أو خلصنا الفحص، بنظهر الزراير عادي
        setIsLoading(false);
      } catch (e) {
  ...
  // بتشتغل لما الأب يدوس على زرار Parent
  const handleParentPress = async () => {
    try {
      ...
      const isParent = userRole === 'parent' || userType === 'parent';

      if (userToken && isParent) {
        // الأب متسجل دخول بالفعل، نوديه لنفس الصفحة اللي بتظهر بعد اللوجن
        // (هي اللي بتخيّره بين Dashboard أو Connect with Child)
        router.push('/nextStep');
      } else {
        // مش متسجل، يروح صفحة اللوجن عادي
        router.push('/parentLogin');
      }
    } catch (e) {
      console.log("Error checking parent status", e);
      router.push('/parentLogin');
    }
  };
